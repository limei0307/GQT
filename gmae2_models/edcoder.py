...
class PreModel(nn.Module):
    def __init__(
        ...
        if self.use_vq:
            # # import sys # debug for vector-quantize-pytorch
            # # sys.path.insert(0, '../vector-quantize-pytorch')
            from vector_quantize_pytorch import VectorQuantize, ResidualVQ
            self.vq = ResidualVQ(
            dim = latent_hidden,
            num_quantizers = vq_num_codebook,
            codebook_size = vq_codebook_size,
            kmeans_init = kmeans_init,
            use_cosine_sim = cosine_sim,
        ...
        if encoder_type in ("gat",):
            ...
            enc_latent_hidden = latent_hidden // nhead
            enc_nhead = nhead
        else:
            enc_num_hidden = num_hidden
            enc_nhead = 1

        dec_in_dim = latent_hidden
        ...
        self.projector = nn.Sequential(
            nn.Linear(latent_hidden, 256),
            nn.PReLU(),
            nn.Linear(256, latent_hidden),
        )
        ...
        self.predictor = nn.Sequential(
            nn.PReLU(),
            nn.Linear(latent_hidden, latent_hidden)
        )
    ...
    def mask_attr_prediction(self, g, x, targets, epoch, drop_g1=None, drop_g2=None):
        ...
        if self.use_dgi:
            permutation = torch.randperm(use_x.shape[0])
            enc_rep_neg = self.encoder(use_g, use_x[permutation])
        
        if self.use_vq:
            enc_rep, _, commit_loss = self.vq(torch.unsqueeze(enc_rep,0), freeze_codebook=False)
            enc_rep = enc_rep[0]
            if self.use_dgi:
                enc_rep_neg, _, commit_loss_neg = self.vq(torch.unsqueeze(enc_rep_neg,0), freeze_codebook=False)
                enc_rep_neg = enc_rep_neg[0]
        if self.use_dgi:
            summary = enc_rep.sum(dim=0)
            dgi_loss = self.dgi_model.loss(enc_rep, enc_rep_neg, summary)
        ...
        if self.use_vq:
            if self.vq_method == 'rq':
                commit_loss = torch.sum(commit_loss)
            loss += commit_loss
            # print('loss_rec, loss_latent, commit_loss:', loss_rec, loss_latent, commit_loss)
            # check relative loss values
        if self.use_dgi:
            loss += dgi_loss
    ...
    def embed(self, g, x):
        ...
        if self.use_vq:
            rep_quantized, indices, _ = self.vq(torch.unsqueeze(rep,0), freeze_codebook=False)
            return rep, rep_quantized[0], indices[0]
