...
def rocauc(y_pred, y_true):
    ...
    rocauc_list = []
    y_true = y_true.unsqueeze(dim=1).detach().cpu().numpy()
    if y_true.shape[1] == 1:
        # use the predicted class for single-class classification
        y_pred = nn.functional.softmax(y_pred, dim=-1)[:,1].unsqueeze(1).cpu().numpy()
    else:
...
def build_args():
    ...
    parser.add_argument('--vq_codebook_size', type=int, default=256, help='')
    ...
    parser.add_argument("--ego_graph_file_path", type=str, default='GraphMAE2/lc_ego_graphs')
    parser.add_argument("--data_dir", type=str, default="GraphMAE2/dataset")
